function delier_doc_reglement(ref_doc, ref_reglement) {
    var AppelAjax = new Ajax.Request(
        "documents_reglements_delier.php", {
            parameters: {
                ref_doc: ref_doc,
...
function liaisons_insert_pack(ref_doc, debut_pack, pack_size) {
    ...
    for (i = debut_pack; i < debut_pack + pack_size; i++) {
        addlinespack[$("liaison_ref_article_" + i).value] = $("new_art_qte_" + i).value;
    }
...
function test_article_pointage(code_barre) {

    var AppelAjax = new Ajax.Updater(
        "pointage_qte_parcouru",
        "test_article_pointage.php", {
            parameters: {
...
function voir_detail_pointage() {

    var AppelAjax = new Ajax.Updater(
        "result_detail_pointage",
        "voir_detail_pointage.php", {
            parameters: {},
            evalScripts: true,
            onLoading: S_loading,
            onSuccess: H_loading
...
function remise_pointage_a_zero(ref_doc) {

    var AppelAjax = new Ajax.Updater(
        "qte_totale_pointage",
        "remise_pointage_a_zero.php", {
            parameters: {
                ref_doc: ref_doc
            },
            evalScripts: true,
...
function maj_pop_up_calcul_qte(id_article, id_doc_line, target) {
    ...
    var AppelAjax = new Ajax.Updater(
        "calcul_qte",
        "documents_line_calcul_qte.php", {
            parameters: {
                ref_article: id_article,
                ref_doc_line: id_doc_line,
                cible: target
            },
            evalScripts: true,
            onLoading: S_loading,
...
//ajout d'un d'article + un parametre
function add_new_line_article_plus(ref_doc, ref_article, qte_article, num_serie, code_barre) {
    var AppelAjax = new Ajax.Request(
        "documents_line_add.php", {
            parameters: {
                ref_doc: ref_doc,
...
//ajout Timbre fiscal
function add_timbre_fiscal(ref_doc) {
    var AppelAjax = new Ajax.Request(
        "documents_add_timpre_fiscal.php", {
            parameters: {
                ref_doc: ref_doc
            },
            evalScripts: true,
...
//Supprimer Timbre fiscal
function sup_timbre_fiscal(ref_doc) {
    var AppelAjax = new Ajax.Request(
        "documents_sup_timpre_fiscal.php", {
            parameters: {
                ref_doc: ref_doc
            },
            evalScripts: true,
    ...
    var AppelAjax = new Ajax.Request(
        "documents_line_add.php", {
            parameters: {
                ref_doc: ref_doc,
                type_of_line: type_of_line,
                id_doc_info_line: id_doc_info_line
            },
            evalScripts: true,
            onLoading: S_loading,
            onException: function() {
...
function article_maj_from_moteur(ref_article, type) {
    ...
    switch (type) {
        case "blur":
            if (($("new_moteur_article_" + ref_article).value == "1") && ($("qte_moteur_article_" + ref_article).value == "0")) {} else if ($("new_moteur_article_" + ref_article).value == "1") {
...
function docu_maj_contact_infos(ref_doc, id_info_content) {
    var AppelAjax = new Ajax.Request(
        "documents_contact_maj_infos.php", {
            parameters: {
                ref_doc: ref_doc,
                id_info_content: id_info_content,
                info_content: escape($(id_info_content).value)
            },
            evalScripts: true,
            onLoading: S_loading,
            onException: function() {
...
function pre_start_adresse_doc(survol, bt_survol, ref_contact, lib_adresse, user_adresse, choix_adresse, iframe_adresse, pagecible, ref_doc, type_adresse) {
    ...
    Event.observe(survol, "mouseover", function() {
        $(bt_survol).src = dirtheme + "images/bt_doc_choix_adresses_hover.gif";
    }, false);
    Event.observe(survol, "mousedown", function() {
        $(bt_survol).src = dirtheme + "images/bt_doc_choix_adresses_down.gif";
    }, false);
    Event.observe(survol, "mouseup", function() {
        $(bt_survol).src = dirtheme + "images/bt_doc_choix_adresses.gif";
    }, false);
    Event.observe(survol, "mouseout", function() {
        $(bt_survol).src = dirtheme + "images/bt_doc_choix_adresses.gif";
    }, false);

    //affichage des choix
    Event.observe(survol, "click", function(evt) {
        Event.stop(evt);
        start_adresse_doc(ref_contact, lib_adresse, user_adresse, choix_adresse, iframe_adresse, pagecible, ref_doc, type_adresse);
    }, false);
        ...
        var AppelAjax = new Ajax.Updater(
            ...
            targeturl, {
                parameters: {
                    ref_contact: ref_contact,
                    choix_adresse: cible,
                    iframe_choix_adresse: iframecible,
                    input: idinput,
                    textarea: idtextarea,
                    ref_doc: ref_doc,
    ...
    var AppelAjax = new Ajax.Request(
        "documents_line_maj_ref_externe.php", {
            parameters: {
                ...
                old_ref_externe: old_ref_externe,
...
function doc_line_maj_ordre(indent_ligne) {
    ...
    var oldordre = parseInt($("ordre_" + id_liste.split("_")[1]).value);
    var newordre = 1;
    if (!$("lignes").empty()) {
    ...
    var AppelAjax = new Ajax.Request(
        "documents_line_maj_lib_article.php", {
            parameters: {
                ref_doc: ref_doc,
                ref_doc_line: ref_doc_line,
                lib_article: escape($(id_lib_article).value)
            },
            evalScripts: true,
            onLoading: S_loading,
            onException: function() {
    ...
    var AppelAjax = new Ajax.Request(
        "documents_line_maj_desc_article.php", {
            parameters: {
                ref_doc: ref_doc,
                ref_doc_line: ref_doc_line,
                desc_article: escape($(id_desc_article).value)
            },
            evalScripts: true,
            onLoading: S_loading,
            onException: function() {
...
function incremente_sn(indentation_art, indentation_sn) {
    ...
    if (nombre_ligne_sn >= doc_aff_qte_sn) {
        nombre_ligne_sn = doc_aff_qte_sn;
    }
...
function pre_start_observer_sn(indentation_art, indentation_sn, ref_doc_line, art_sn, old_art_sn, sup_sn, more_sn, ref_article, choix_div, choix_iframe) {
    ...
    Event.observe(sup_sn, "click", function(evt) {
        Event.stop(evt);
...
//Recalcul du total des qtes par numero de lots
function document_calcul_sn_qte_total(indentation_art) {

    var nl = 0;
    var total_qte = 0;
    while ($("qte_nl_" + indentation_art + "_" + nl)) {
        if ($("qte_nl_" + indentation_art + "_" + nl).value != "") {
            total_qte += parseFloat($("qte_nl_" + indentation_art + "_" + nl).value);
        }
        nl++;
    }
    if ($("qte_" + indentation_art).value == total_qte) {
        $("total_sn_qte_" + indentation_art).innerHTML = total_qte;
    } else {
        $("total_sn_qte_" + indentation_art).innerHTML = "<FONT COLOR='#FF0000'>" + total_qte + "</FONT>";
    }
    ...
    var AppelAjax = new Ajax.Request(
        "documents_line_maj_nl.php", {
            parameters: {
                ...
                nl: nl,
                new_nl: new_nl,
                old_qte_nl: old_qte_nl,
                new_qte_nl: new_qte_nl,
                art_nl: art_nl,
                qte_nl: qte_nl,
                indentation_art: indentation_art,
                indentation_nl: indentation_nl
            },
            evalScripts: true,
            onLoading: S_loading,
            onException: function() {
...
function pre_start_observer_nl(indentation_art, indentation_nl, ref_doc_line, art_nl, old_art_nl, sup_nl, more_nl, ref_article, choix_div, choix_iframe, qte_nl, old_qte_nl) {
        ...
        if (quantite_locked) {
            ...
            if ($(art_nl).value != $(old_art_nl).value) {
                if ($(art_nl).value == "") {
                    ...
                    maj_line_nl(ref_doc_line, $(old_art_nl).value, $(art_nl).value, $(old_qte_nl).value, $(qte_nl).value, art_nl, qte_nl, indentation_art, indentation_nl);
                    $(old_art_nl).value = $(art_nl).value;
                    $(old_qte_nl).value = $(qte_nl).value;
                }
            }
    ...
    Event.observe(qte_nl, "blur", function(evt) {
        if (quantite_locked) {
            ...
            if ($(qte_nl).value != $(old_qte_nl).value && (nummask(evt, $(old_qte_nl).value, "X.X"))) {
                if ($(qte_nl).value == "") {
                    del_line_nl(ref_doc_line, $(old_art_nl).value, $(old_qte_nl).value);
                    $(old_art_nl).value = $(art_nl).value = "";
                    $(old_qte_nl).value = $(qte_nl).value = "";
...
function insert_line_nl(indentation_art, indentation_nl) {
    ...
    $("iframe_liste_choix_nl_" + indentation_art + "_" + indentation_nl).setStyle({
        display: 'none'
    });
    ...
    $("choix_liste_choix_nl_" + indentation_art + "_" + indentation_nl).setStyle({
        display: 'none'
    });
    ...
    if ($(cible).style.display == "none") {
        ...
        var AppelAjax = new Ajax.Updater(
            ...
            targeturl, {
                parameters: {
                    ref_article: ref_article,
                    choix_ref_externe: cible,
                    iframe_choix_ref_externe: iframecible,
                    input: idinput,
                    ref_doc: ref_doc
                },
                evalScripts: true,
...
function insert_line_sn(indentation_art, indentation_sn) {
    ...
    $("iframe_liste_choix_sn_" + indentation_art + "_" + indentation_sn).setStyle({
        display: 'none'
    });
    ...
    $("choix_liste_choix_sn_" + indentation_art + "_" + indentation_sn).setStyle({
        display: 'none'
    });
...
function affichage_sn_update(indentation_art, new_qte, old_qte) {
    ...
    if ($("aff_all_sn_" + indentation_art)) {
        $("aff_all_sn_" + indentation_art).style.display = "none";
    }
    ...
    if (maj_qte > doc_aff_qte_sn) {
        if ($("aff_all_sn_" + indentation_art)) {
            $("aff_all_sn_" + indentation_art).style.display = "";
        }
    }
    if (maj_qte >= doc_aff_qte_sn) {
...
function maj_ref_doc_externe(id_info_content) {
    ...
    var AppelAjax = new Ajax.Request(
        "documents_entete_maj_ref_doc_externe.php", {
            parameters: {
                ref_doc: ref_doc,
                info_content: escape($F(id_info_content))
            },
            evalScripts: true,
            onLoading: S_loading,
            onException: function() {
...
//maj code_affaire
function maj_code_affaire(id_info_content) {
    var ref_doc = $("ref_doc").value;
    var AppelAjax = new Ajax.Request(
        "documents_entete_maj_code_affaire.php", {
            parameters: {
                ref_doc: ref_doc,
                info_content: escape($F(id_info_content))
            },
...
function generer_code_affaire(id_info_content) {
    var AppelAjax = new Ajax.Request(
        "documents_entete_generer_code_affaire.php", {
            ...
            onComplete: function(requester) {
                $(id_info_content).value = requester.responseText;
                H_loading();
            }
        }
    );
...
function generer_new_open_doc(global_option) {
    var ref_doc = $("ref_doc").value;
    var AppelAjax = new Ajax.Updater(
        "sub_content",
        "documents_cotation_gen_new_open_doc.php", {
            parameters: {
                ref_doc: ref_doc,
                global_option: global_option
            },
...
function generer_document(fonction_generer) {
    ...
    var AppelAjax = new Ajax.Updater(
        ...
        "documents_edition_generer.php", {
            parameters: {
                ref_doc: ref_doc,
                fonction_generer: fonction_generer
            },
            evalScripts: true,
            onLoading: S_loading
        }
...
function maj_doc_event(ref_doc_event, date_event, id_event_type, d_event) {
    ...
    var AppelAjax = new Ajax.Request(
        "documents_events_maj.php", {
            parameters: {
                ref_doc: ref_doc,
                ref_doc_event: $(ref_doc_event).value,
                date_event: $(date_event).value,
                id_event_type: $(id_event_type).value,
                d_event: $(d_event).value
            },
            evalScripts: true,
            onLoading: S_loading,
            onException: function() {
...
function maj_entete_id_stock_source(id_stock_source) {
    ...
    var AppelAjax = new Ajax.Request(
        "documents_entete_maj_id_stock_source.php", {
            parameters: {
                ref_doc: ref_doc,
                id_stock_source: $(id_stock_source).value
            },
            evalScripts: true,
            onLoading: S_loading,
            onException: function() {
...
function maj_entete_id_stock_cible(id_stock_cible) {
    ...
    var AppelAjax = new Ajax.Request(
        "documents_entete_maj_id_stock_cible.php", {
            parameters: {
                ref_doc: ref_doc,
                id_stock_cible: $(id_stock_cible).value
            },
            evalScripts: true,
            onLoading: S_loading,
            onException: function() {
...
function maj_id_niveau_relance(id_info_content) {
    ...
    var AppelAjax = new Ajax.Updater(
        ...
        "documents_entete_maj_id_niveau_relance.php", {
            parameters: {
                ref_doc: ref_doc,
                info_content: $(id_info_content).value
            },
            evalScripts: true,
            onLoading: S_loading,
            onException: function() {
...
function document_calcul_tarif(bool) {
    ...
    var qte_toto = 0;
    // timbre fiscal
    if ($("timbre_fiscal").value == "oui") {
        timbre_fiscal = $("valeur_timbre").value;
    }
    ...
    if (!$("lignes").empty()) {
        ...
        for (i = 0; i < t_liste.length; i++) {
            // TODO Max
            maj_lignes_liees(t_liste[i], false);
            ...
            if ($("pt_" + t_liste[i])) {
                ...
                if (app_tarif == "HT") {
                    ...
                    $("pt_" + t_liste[i]).value = (Math.round((prix_u * qte_a) * remise_a * 1000) / 1000).toFixed(tarifs_nb_decimales);
                    ...
                    $("pt_" + t_liste[i]).value = (Math.round(((prix_u * qte_a) * remise_a) * tva_a * 1000) / 1000).toFixed(tarifs_nb_decimales);
                }
            ...
            if ($("ss_toto_" + t_liste[i])) {
                $("ss_toto_qte" + t_liste[i]).innerHTML = qte_toto;
                if ($("prix_afficher_ht").checked) {
    ...
    // controle sur le timbre fiscal pt_ttc 

    pt_ttc = parseFloat(pt_ttc) + parseFloat(timbre_fiscal);
    ...
    $("toto_timbre").innerHTML = parseFloat(timbre_fiscal).toFixed(tarifs_nb_decimales) + " " + monnaie_html;
    ...
    if ($("montant_total_reglement")) {
        ...
        $("montant_total_reglement").innerHTML = (Math.round(parseFloat(pt_ttc) * 1000) / 1000).toFixed(tarifs_nb_decimales);
        if ($("montant_total_reglement2")) {
            $("montant_total_reglement2").innerHTML = (Math.round(parseFloat(pt_ttc) * 1000) / 1000).toFixed(tarifs_nb_decimales);
        }
...
action_line_coche.prototype = {
        ...
        if (!$("lignes").empty()) {
            ...
            if (this.lignes.length > 0) {
                ...
                switch (this.type_action) {
                    case "imprimer_etiquettes2":

                        this.imprimer_etiquette2();
                        break;
                    case "imprimer_etiquettes_solde":
                        this.imprimer_etiquette_solde();
                        break;
    ...
    imprimer_etiquette2: function() {
        ...
        page.verify("catalogue_impression_etiquettes50_30.php", "catalogue_impression_etiquettes50_30.php?" + send_param, "true", "_blank");

    },


    imprimer_etiquette_solde: function() {
        this.lignes = new Array();
        if (!$("lignes").empty()) {
            this.liste = serialisation("lignes", "li").replace(/lignes\[\]=/g, "");
            t_liste = this.liste.split("&");
        ...
        page.verify("catalogue_impression_etiquettes_solde.php", "catalogue_impression_etiquettes_solde.php?" + send_param, "true", "_blank");

    },
    imprimer_etiquette: function() {
        this.lignes = new Array();
        if (!$("lignes").empty()) {
            this.liste = serialisation("lignes", "li").replace(/lignes\[\]=/g, "");
            t_liste = this.liste.split("&");
        ...
        page.verify("catalogue_impression_etiquettes.php", "catalogue_impression_etiquettes.php?" + send_param, "true", "_blank");
    ...
    var AppelAjax = new Ajax.Updater(
        ...
        "documents_nouveau_copie_line.php?" + send_param, {
            parameters: {
                ...
                link_old_ref_doc: link_old_ref_doc
            },
            evalScripts: true,
            onLoading: S_loading,
            onException: function() {
...
function link_document_from(ref_doc, ref_doc_from) {
    var AppelAjax = new Ajax.Request(
        "documents_edition_block_liaisons_link.php", {
            parameters: {
                ref_doc_cible: ref_doc,
                ref_doc_liaison_pos: ref_doc_from
            },
            evalScripts: true,
            onLoading: S_loading,
            onSuccess: function(requester) {
...
function pre_start_article_line(ref_article, ref_doc_line, indentation) {
    ...
    Event.observe("link_to_art_" + indentation, "click", function(evt) {
        Event.stop(evt);
        page.verify('catalogue_articles_view', 'index.php#' + escape('catalogue_articles_view.php?ref_article=' + ref_article), 'true', '_blank');
    }, false);
    ...
    Event.observe("der_aff_" + indentation, "click", function(evt) {
        if ($("der_aff_val_" + indentation).value == "1") {
            $("qte_aff_" + indentation).hide();
            $("pu_aff_" + indentation).hide();
            $("remise_aff_" + indentation).hide();
            $("tva_aff_" + indentation).hide();
            $("pt_aff_" + indentation).hide();
            $("der_aff_val_" + indentation).value = "0";
        } else {
            $("qte_aff_" + indentation).show();
            $("pu_aff_" + indentation).show();
            $("remise_aff_" + indentation).show();
            $("tva_aff_" + indentation).show();
            $("pt_aff_" + indentation).show();
            $("der_aff_val_" + indentation).value = "1";
        }


    }, false);

    Event.observe("stock_aff_" + indentation, "click", function(evt) {
        show_resume_stock(ref_article, evt);
        Event.stop(evt);
    }, false);

    Event.observe("tarif_aff_" + indentation, "click", function(evt) {
        show_resume_tarif(ref_article, evt);
        Event.stop(evt);
    }, false);
    ...
    Event.observe("qte_" + indentation, "blur", function(evt) {
        if (quantite_locked) {
            ...
            if (($("qte_" + indentation).value != $("qte_old_" + indentation).value) && (nummask(evt, $("qte_old_" + indentation).value, "X.X"))) {
                ...
                // On modifie les quantités des lignes liées
                maj_lignes_liees(indentation, true);
                $("qte_old_" + indentation).value = $("qte_" + indentation).value;
                //	document_calcul_tarif ();
            }
            if ($("art_gest_nl_" + indentation)) {
                document_calcul_sn_qte_total(indentation);
            }
        }
    }, false);
    ...
    Event.observe("link_" + indentation, "click", function(evt) {
        ...
        if (quantite_locked) {
            ...
            maj_pop_up_calcul_qte(ref_article, ref_doc_line, "qte_" + indentation);
            maj_pop_up_calcul_prix();
            $("pop_up_options").style.display = "block";
        }
    }, false);
...
function insert_line_fab_nl(indentation_nl, ref_article, ref_doc) {
    ...
    $("iframe_liste_choix_nl_" + indentation_nl).setStyle({
        display: 'none'
    });
    ...
    $("choix_liste_choix_nl_" + indentation_nl).setStyle({
        display: 'none'
    });
...
function check_document_compta_lignes() {
    ...
    for (var j = 0; j < indent; j++) {
        if ($("montant_" + j) && !$("montant_" + j).disabled) {
            if ($("id_journal_" + j).value == "3") {
                total_lines_ht += parseFloat($("montant_" + j).value);
            }
            if ($("id_journal_" + j).value == "4") {
                total_lines_tva += parseFloat($("montant_" + j).value);
            }
            if ($("id_journal_" + j).value == "5") {
                total_lines_ttc += parseFloat($("montant_" + j).value);
            }
            if ($("id_journal_" + j).value == "6") {
                total_lines_ht += parseFloat($("montant_" + j).value);
            }
            if ($("id_journal_" + j).value == "7") {
                total_lines_tva += parseFloat($("montant_" + j).value);
            }
            if ($("id_journal_" + j).value == "8") {
                total_lines_ttc += parseFloat($("montant_" + j).value);
            }
...
/*
 * Fonction permettant de mettre à jour les lignes liées à une ligne (composition, taxes, ...)
 */
function maj_lignes_liees(indentation, maj_base) {
    // On MAJ les lignes liées
    if ($("lignesliees_" + indentation)) {
        var nb = parseInt($("lignesliees_" + indentation).value);
        for (indice = 1; indice <= nb; indice++) {
            var indent = parseInt(indentation) + indice;
            if ($("qte_" + indent)) {
                if ($("lignesliees_" + indent)) { // Si la ligne est de type article
                    $("qte_old_" + indent).value = $("qte_" + indent).value;
                    qte = parseInt($("qte_" + indentation).value);
                    old_qte = parseInt($("qte_old_" + indent).value);
                    old_qte_parent = parseInt($("qte_old_" + indentation).value);
                    $("qte_" + indent).value = qte * (old_qte / old_qte_parent);
                } else { // Sinon (la ligne est de type taxe)
                    $("qte_" + indent).value = $("qte_" + indentation).value;
                }
                if ($("pt_" + indent)) {
                    qte = parseInt($("qte_" + indent).value);
                    pu_ht = parseFloat($("pu_" + indent).value);
                    $("pt_" + indent).value = price_format(qte * pu_ht);
                }
                if (maj_base == true) {
                    maj_line_qte($("qte_" + indent).value, $("ref_doc_line_" + indent).value,
                        $("ref_article_" + indent).value, indent);
                }
            }
        }
    }
...
function doc_modifier_echeance(ref_doc, montant_acquite) {
    var AppelAjax = new Ajax.Updater(
        'alert_pop_up_tab_echeancier',
        'documents_edition_modifier_echeance.php', {
            parameters: {
                ref_doc: ref_doc,
                montant_acquite: montant_acquite
            },
            evalScripts: true,
            onLoading: S_loading,
            onSuccess: function(requester) {
    ...
    $('alert_pop_up_tab_echeancier').style.display = "block";
    $("alert_pop_up").style.display = "block";
}

function doc_ajouter_echeance(ref_doc, montant_acquite, montant_ech, select_montant_ech, mode_reglement_ech, date_fixe_ech, date_delai_ech, type_ech) {
    var AppelAjax = new Ajax.Request(
        'documents_edition_ajouter_echeance.php', {
            parameters: {
                ref_doc: ref_doc,
                montant_acquite: montant_acquite,
                montant_ech: montant_ech,
                select_montant_ech: select_montant_ech,
                mode_reglement_ech: mode_reglement_ech,
                date_fixe_ech: date_fixe_ech,
                date_delai_ech: date_delai_ech,
                type_ech: type_ech
            },
            evalScripts: true,
            onLoading: S_loading,
...
function doc_supprimer_echeance(ref_doc, montant_acquite) {
    var AppelAjax = new Ajax.Request(
        'documents_edition_supprimer_echeance.php', {
            parameters: {
                ref_doc: ref_doc,
                montant_acquite: montant_acquite
            },
...
function calcul_modification_duree(indent, ref_article, ref_doc_line, date_deb_abo, duree_mois_abo, duree_jours_abo) {
    if (ref_doc_line != "" && date_deb_abo != "" && duree_mois_abo != "" && duree_jours_abo != "") {
        var AppelAjax = new Ajax.Request(
            'documents_line_article_calcul_modification_duree.php', {
                parameters: {
                    indentation: indent,
                    ref_article: ref_article,
                    ref_doc_line: ref_doc_line,
                    date_deb_abo: date_deb_abo,
                    duree_mois_abo: duree_mois_abo,
                    duree_jours_abo: duree_jours_abo
                },
                evalScripts: true,
                onLoading: S_loading,
                onSuccess: function(requester) {
...
function verifier_qte() {
    //alert('test3');
    // test sur les quantité
    if (document.getElementById('sans_varaiant')) {
        if (document.getElementById('quantite').value == '') {
            alert('veuillez indiquer la quantité');
        } else {
            document.getElementById("article_add_parlot").submit();
        }
    } else {
        var nb_art = document.getElementById('indentations_variantes').value;
        var qte_existe = false;
        for (i = 0; i < nb_art; i++) {
            if (document.getElementById('qte_' + i).value != '') {
                qte_existe = true;
                break;
                alert(document.getElementById('qte_' + i).value);
            }
        }
        if (qte_existe == true) {
            document.getElementById("article_add_parlot").submit();
        } else {
            alert('veuillez indiquer une quantité au minimum');
        }
    }
}

function verifier_qte_parent_existe() {
    //alert('test3');
    // test sur les quantité
    if (document.getElementById('sans_varaiant')) {
        if (document.getElementById('quantite').value == '') {
            ...
            document.getElementById("article_add_parlot_parent_existe").submit();
        }
    } else {
        var nb_art = document.getElementById('indentations_variantes').value;
        var qte_existe = false;
