...
function supp_new_caiu(id_bt, id_li) {
    Event.observe(id_bt, 'click', function(evt) {
        Event.stop(evt);
        remove_tag(id_li);
    }, false);
...
function pre_start_commune(idcode, idcommune, cible, iframecible, targeturl, id_pays) {
    ...
    Event.observe(idcommune, 'focus', function() {
        start_commune(idcode, idcommune, cible, iframecible, targeturl);
    }, false);
    Event.observe(id_pays, 'focus', function() {
        ...
        var AppelAjax = new Ajax.Updater(
            ...
            targeturl + cp, {
                parameters: {
                    ville: idcommune,
                    choix_ville: cible,
                    iframe_choix_ville: iframecible
                },
                evalScripts: true,
                onLoading: S_loading,
...
function pre_start_coordonnee(survol, bt_survol, ref_contatct, lib_coord, user_coord, choix_coord, iframe_coord, pagecible) {
    ...
    Event.observe(survol, "mouseover", function() {
        $(bt_survol).src = dirtheme + "images/bt-arrow_select_hover.gif";
    }, false);
    Event.observe(survol, "mousedown", function() {
        $(bt_survol).src = dirtheme + "images/bt-arrow_select_down.gif";
    }, false);
    Event.observe(survol, "mouseup", function() {
        $(bt_survol).src = dirtheme + "images/bt-arrow_select.gif";
    }, false);
    ...
    Event.observe(survol, "click", function(evt) {
        Event.stop(evt);
        start_coordonnee(ref_contatct, lib_coord, user_coord, choix_coord, iframe_coord, pagecible);
    }, false);
        ...
        var AppelAjax = new Ajax.Updater(
            ...
            targeturl, {
                parameters: {
                    ...
                    choix_coord: cible,
                    iframe_choix_coord: iframecible,
                    input: idinput,
                    div: iddiv
                },
                evalScripts: true,
...
function pre_start_adresse_crea(survol, bt_survol, lib_adresse, user_adresse, choix_adresse, iframe_adresse) {
    ...
    Event.observe(survol, "click", function(evt) {
        Event.stop(evt);
        start_adresse_crea(lib_adresse, user_adresse, choix_adresse, iframe_adresse);
    }, false);
...
function evenement_listechoix_adresse(texte_racourci, iddiv, iframecible, cible, idinput, i, ordre) {
    Event.observe("li_choix_adresse_" + idinput + "_" + i, "mouseout", function() {
        Element.removeClassName($("li_choix_adresse_" + idinput + "_" + i), "complete_coordonnee_hover");
        Element.addClassName($("li_choix_adresse_" + idinput + "_" + i), "complete_coordonnee");
    }, false);

    Event.observe("li_choix_adresse_" + idinput + "_" + i, "mouseover", function() {
        Element.addClassName($("li_choix_adresse_" + idinput + "_" + i), "complete_coordonnee_hover");
        Element.removeClassName($("li_choix_adresse_" + idinput + "_" + i), "complete_coordonnee");
    }, false);

    Event.observe("li_choix_adresse_" + idinput + "_" + i, "click", function(evt) {
        Event.stop(evt);
        $(idinput).value = ordre;
        $(cible).style.display = "none";
        $(iframecible).style.display = "none";
        $(iddiv).innerHTML = texte_racourci;
    }, false);
}
...
function pre_start_adresse(survol, bt_survol, ref_contatct, lib_adresse, user_adresse, choix_adresse, iframe_adresse, pagecible) {
    ...
    Event.observe(survol, "click", function(evt) {
        Event.stop(evt);
        start_adresse(ref_contatct, lib_adresse, user_adresse, choix_adresse, iframe_adresse, pagecible);
    }, false);
...
function nummask(evt, val_def, masque) {
    ...
    if ($(id_field.id) && ($(id_field.id).value == "" || $(id_field.id).value == "NaN")) {
        $(id_field.id).value = val_def;
        to_return = false;
    } else {
        to_return = true;
    }
...
function changeref_art_categ3() {
    var AppelAjax = new Ajax.Request(
        "catalogue_articles_edition_test_modele_info.php", {
            parameters: {
                ref_art_categ: $("mod_ref_art_categ").value,
                old_ref_art_categ: $("old_ref_art_categ").value
            },
            evalScripts: true,
            onLoading: S_loading,
            onException: function() {
...
function pre_start_art_categ(ref_art_categ, lib_art_categ, inpendice) {
    Event.observe('tr_' + ref_art_categ + inpendice, 'mouseover', function() {
        changeclassname('tr_' + ref_art_categ + inpendice, 'list_art_categs_hover');
    }, false);

    Event.observe('tr_' + ref_art_categ + inpendice, 'mouseout', function() {
        changeclassname('tr_' + ref_art_categ + inpendice, 'list_art_categs');
    }, false);
...
function pre_start_art_categ2(ref_art_categ, lib_art_categ, inpendice) {
    ...
    Event.observe('mod_' + ref_art_categ + inpendice, 'click', function(evt) {
        ...
        $("lib_art_categ" + inpendice).innerHTML = lib_art_categ;
        charger_carac_simple(ref_art_categ, "caract_simple");
    }, false);
}
...
function pre_start_art_categ3(ref_art_categ, lib_art_categ, inpendice) {
    ...
    Event.observe('mod_' + ref_art_categ + inpendice, 'click', function(evt) {
        Event.stop(evt);
        $("mod_ref_art_categ" + inpendice).value = ref_art_categ;
...
function pre_start_art_categ4(ref_art_categ, lib_art_categ, inpendice) {
    Event.observe('tr_inv_' + ref_art_categ + inpendice, 'mouseover', function() {
        changeclassname('tr_inv_' + ref_art_categ + inpendice, 'list_art_categs_hover');
    }, false);

    Event.observe('tr_inv_' + ref_art_categ + inpendice, 'mouseout', function() {
        changeclassname('tr_inv_' + ref_art_categ + inpendice, 'list_art_categs');
    }, false);

    Event.observe('mod_inv_' + ref_art_categ + inpendice, 'click', function(evt) {
        Event.stop(evt);
        $("new_inv_ref_art_categ" + inpendice).value = ref_art_categ;
        Element.toggle('liste_de_categorie_selectable' + inpendice);
        Element.toggle('iframe_liste_de_categorie_selectable' + inpendice);
        $("lib_art_categ" + inpendice).innerHTML = lib_art_categ;
    }, false);
...
function tb2_aff_resultat_1() {
    ...
    var AppelAjax = new Ajax.Updater(
        ...
        "tableau_bord_ventes_tb2_contener2.php", {
            parameters: {
                magazin: $("magazin_rech").value,
                date_debut: $("date_debut").value,
                date_fin: $("date_fin").value,
                type_progress: $("type_progress").value
            },
            evalScripts: true,
            onLoading: S_loading,
            onException: function() {
...
function tb3_aff_resultat_1() {

    var AppelAjax = new Ajax.Updater(
        "tb2_det_aff",
        "tableau_bord_ventes_tb3_contener3.php", {
            parameters: {
                date_debut: $("date_debut").value,
                date_fin: $("date_fin").value,
                type_progress: $("type_progress").value,
                id_magazin: $("magazin_tb3").options[$("magazin_tb3").selectedIndex].value
            },
            evalScripts: true,
            onLoading: S_loading,
            onException: function() {
        ...
        switch (type_progress) {
            case "J":
                ...
                if (tab[1].substr(0, 1) == '0') {
                    tab[1] = tab[1].substr(1, 2);
                }
                if (tab[0].substr(0, 1) == '0') {
                    tab[0] = tab[0].substr(1, 2);
                }
    ...
    var AppelAjax = new Ajax.Updater(
        ...
        "documents_historique_ventes_content.php", {
            parameters: {
                ...
                type_progress: $("type_progress").value,
                id_stock: "",
                type_values: $("type_values").value,
                id_magazin: $("magazin_tb4").options[$("magazin_tb4").selectedIndex].value,
                ref_contact: $("ref_contact_s").value
            },
            evalScripts: true,
            onLoading: S_loading,
            onException: function() {
...
function tb5_aff_resultat_xz(page_to_show_s) {

    var AppelAjax = new Ajax.Updater(
        "tb5_det_aff",
        "documents_recette_xz_content.php", {
            parameters: {
                page_to_show: page_to_show_s,
                date_debut: $("date_debut").value,
                date_fin: $("date_fin").value,
                type_progress: $("type_progress").value,
                id_stock: "",
                type_values: $("type_values").value,
                orderby_s: $("orderby_s").value,
                orderorder: $("orderorder").value,
                magazin: $("magazin").options[$("magazin").selectedIndex].value,
                aff_prix: $("aff_prix").value,
                ref_art_categ: $("ref_art_categ_tbv").value,
                lib_article: $("lib_article_tbv").value
            },
            evalScripts: true,
            onLoading: S_loading,
            onException: function() {
...
// impression de recette xz
function recette_xz_imprimer() {

    var nom_magazin = $("magazin").options[$("magazin").selectedIndex].value;
    window.open('recette_xz_editing.php?date_debut=' + $("date_debut").value + '&date_fin=' + $("date_fin").value + '&orderby_s=' + $("orderby_s").value + '&orderorder=' + $("orderorder").value + '&magazin=' + nom_magazin + '&aff_prix=' + $("aff_prix").value + '&ref_art_categ=' + $("ref_art_categ_tbv").value + '&lib_article=' + $("lib_article_tbv").value + '', '_blank');
}

function recette_xz_imprimer1() {

    var nom_magazin = $("magazin").options[$("magazin").selectedIndex].value;
    window.open('recette_xz_editing1.php?date_debut=' + $("date_debut").value + '&date_fin=' + $("date_fin").value + '&orderby_s=' + $("orderby_s").value + '&orderorder=' + $("orderorder").value + '&magazin=' + nom_magazin + '&aff_prix=' + $("aff_prix").value + '&ref_art_categ=' + $("ref_art_categ_tbv").value + '&lib_article=' + $("lib_article_tbv").value + '', '_blank');
}


function historique_xz_imprimer2() {
    window.open('historique_xz_editing2.php?date_debut=' + $("date_debut").value + '&date_fin=' + $("date_fin").value + '&type_values=' + $("type_values").value + '&ref_contact=' + $("ref_contact_s").value + '&id_magazin=' + $("magazin_tb4").options[$("magazin_tb4").selectedIndex].value + '', '_blank');
}