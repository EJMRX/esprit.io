...
function add_image_fils(ref_article) {
...
function maj_info_dormant(ref_article, jour, qte) {

    var AppelAjax = new Ajax.Request(
        "maj_info_dormant.php", {
            parameters: {
                ref_article: ref_article,
                jour: jour,
                qte: qte
    ...
    Event.observe("ratio_" + num_serie, "blur", function(evt) {
        ...
        if (direction_liaison == "vers") {
            ...
            var AppelAjax = new Ajax.Request(
                "catalogue_articles_edition_liaison_maj.php", {
                    parameters: {
                        ref_article_A: ref_article_B,
                        ref_article_B: ref_article_A,
                        id_liaison_type: id_liaison_type,
                        ratio: $("ratio_" + num_serie).value
                    },
                    evalScripts: true,
...
function appel_li_composant(niveau, serie_comp, id_article, lib_article, valo_indice) {
    ...
    var AppelAjax = new Ajax.Updater(
        ...
        'catalogue_articles_composant_li.php?niveau=' + niveau + "&serie_composant=" + $('serialisation_composant').value, {
            ...
            onComplete: function() {
                H_loading();
                Sortable.create('composant_ul', {
                    dropOnEmpty: true,
                    ghosting: true,
                    scroll: 'lot_info',
                    handle: 'composant_li_lib_handle'
                });
            },
    ...
    var AppelAjax = new Ajax.Updater(
        ...
        'catalogue_articles_edition_composant_li.php?niveau=' + niveau + "&serie_composant=" + $('serialisation_composant').value, {
            parameters: {
                ...
                lib_article: lib_article,
                valo_indice: valo_indice,
                ref_article_parent: $("ref_article").value
            },
            evalScripts: true,
            onLoading: S_loading,
            onException: function() {
...
function verif_composant_valo(evt, valo_id) {
    ...
    if (parseInt(temp_result / $(valo_id).value) == (temp_result / $(valo_id).value)) {
...
function stop_view_categ_carac_parent() {
    $('pop_up_view_categ_carac_iframe_parent').hide();
    $('pop_up_view_categ_carac_parent').hide();

}
//fonction de retour du choix
function show_view_categ_carac_parent() {
    // $('pop_up_view_categ_carac_iframe_parent').style.display='block';
    $('pop_up_view_categ_carac_parent').style.display = 'block';
    ...
    var AppelAjax = new Ajax.Request(
        "catalogue_edition_maj_emplacement_stock.php", {
            parameters: {
                ref_article: ref_article,
                id_stock: id_stock,
                new_emplacement: new_emplacement
            },
            evalScripts: true,
            onLoading: S_loading,
            onException: function() {
...
function calcul_tarif_cell_marge(num_ligne_tarif, num_ligne_qte) {
    ...
    var AppelAjax = new Ajax.Request(
        "catalogue_articles_tarifs_marges.php", {
            parameters: {
                formule: escape($("formule_tarif_" + num_ligne_tarif + "_" + num_ligne_qte).value),
                qte: escape($("qte_tarif_" + num_ligne_qte).value),
                div_cible: "show_info_marge_" + num_ligne_tarif + "_" + num_ligne_qte,
                prix_a: pa,
                prix_p: pp,
                tva: tarif_tva
            },
...
function art_edition_fusion(second_ref_article, ref_article) {
    var AppelAjax = new Ajax.Request(
        'catalogue_articles_fusion.php', {
            parameters: {
                ref_article: ref_article,
                second_ref_article: second_ref_article
            },
            evalScripts: true,
            onLoading: S_loading,
            onSuccess: function(requester) {
...
function insert_art_inventory_line_sn(indentation_sn) {
    ...
    $("iframe_liste_choix_sn_" + indentation_sn).setStyle({
        display: 'none'
    });
    ...
    $("choix_liste_choix_sn_" + indentation_sn).setStyle({
        display: 'none'
    });
...
function start_choix_sn_inventory(ref_article, idinput, cible, iframecible, targeturl) {
    if ($(cible).style.display == "none") {
        var id_stock = "";
        if ($("id_stock_inventory")) {
            id_stock = $("id_stock_inventory").value;
        }
...
function charger_variations_possibles_parent(ref_art_categ) {
...
function charger_variations_possibles_parlot_parent(ref_art_categ) {
    ...
    var AppelAjax = new Ajax.Updater(
        "variantes_info_under_parent",
        "catalogue_articles_variantes_parlot.php", {
            method: 'post',
            contentType: 'application/x-www-form-urlencoded',
            asynchronous: true,
            parameters: (tquerystring).toQueryParams(),
...
function charger_edition_variations_possibles_parent(ref_art_categ, ref_article) {
    var variation_car = new Array;
    var indent_var = 0;
    var tquerystring = "ref_art_categ=" + ref_art_categ;
    ...
    var AppelAjax = new Ajax.Updater(
        "variantes_info_under_parent",
        "catalogue_articles_edition_variantes_parent.php", {
            method: 'post',
            asynchronous: true,
            contentType: 'application/x-www-form-urlencoded',
            parameters: tquerystring.toQueryParams(),
...
function charger_variations_possibles_parlot(ref_art_categ) {
...
function charger_variations_possibles_parlot_with_ckeckbox(ref_art_categ, affiche_code_barre, affiche_prix_achat, affiche_prix_public) {

    var variation_car = new Array;
    var indent_var = 0;
    var tquerystring = "ref_art_categ=" + ref_art_categ;
    tquerystring += "&affiche_code_barre=" + affiche_code_barre + "&affiche_prix_achat=" + affiche_prix_achat + "&affiche_prix_public=" + affiche_prix_public;
    var variation_compte = 0;
    for (i = 0; i < parseFloat($("serialisation_carac").value); i++) {
        if ($("variante_" + i).value == "1" && $("caract_value_" + i).value != "") {
            tquerystring += "&" + $("ref_carac_" + i).value + "=" + ($("caract_value_" + i).value);
