...
function tache_reload_crees() {
    ...
    if ($("order_by_date")) {
        order_by_date = $("order_by_date").value;
    }
    if ($("order_by_urgence")) {
        order_by_urgence = $("order_by_urgence").value;
    }
    if ($("order_by_importance")) {
        order_by_importance = $("order_by_importance").value;
    }
    if ($("all_etat_tache").checked) {
        all_etat_tache = "1";
    }
    ...
    var AppelAjax = new Ajax.Updater(
        ...
        "planning_taches_crees_liste.php", {
            parameters: {
                all_etat_tache: all_etat_tache,
                page_to_show: page_to_show,
                order_by_date: order_by_date,
                order_by_urgence: order_by_urgence,
...
function pre_start_sup_collab_tache_edition(id_tache, id_bt, id_tag, ref_contact) {
    Event.observe(id_bt, "click", function(evt) {
        Event.stop(evt);
        del_collab_tache(id_tache, id_tag, ref_contact);
    }, false);
