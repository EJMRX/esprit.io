...
//***************************************************************
function archiver_reg_cheq(date_saisie, montant_reg, ref_reg, retour) {

    var AppelAjax = new Ajax.Updater(
        retour,
        "archiver_reglement_cheque.php", {
            parameters: {
                date_saisie: date_saisie,
                montant_reg: montant_reg,
                ref_reg: ref_reg
            },
            evalScripts: true,
            onLoading: S_loading,
...
function imprimer_chq_entreprise() {
    var chq_en_cours = "0";
    var chq_en_archive = "0";

    if ($F("chq_encours") == "1") {
        chq_en_cours = "1";
    }

    if ($F("chq_enarchives") == "1") {
        chq_en_archive = "1";
    }
    var recherche = '1';
    var id_compte_bancaire = $F('id_compte_bancaire');
    var montant = $("montant").value;
    var conatct = $("ref_contact_s").value;
    var num_cheque = $("num_cheque").value;
    var date_debut = $("date_debut").value;
    var date_fin = $("date_fin").value;
    var debut_echeance = $("debut_echeance").value;
    var fin_echeance = $("fin_echeance").value;
    var delta_montant = $("delta_montant").value;


    var orderby = $F('orderby');
    var orderorder = $F('orderorder');
    if ((chq_en_cours == "0") && (chq_en_archive == "0")) {
        alert("Veuillez cocher chéque en cours ou/et cheque en archive");
    } else {

        window.open('reglement_editing.php?cheq_en_archive=' + chq_en_archive + '&cheq_en_cours=' + chq_en_cours + '&id_compte_bancaire=' + id_compte_bancaire + '&montant=' + montant + '&conatct=' + conatct + '&num_cheque=' + num_cheque + '&date_debut=' + date_debut + '&date_fin=' + date_fin + '&debut_echeance=' + debut_echeance + '&fin_echeance=' + fin_echeance + '&delta_montant=' + delta_montant + '&orderby=' + orderby + '&orderorder=' + orderorder + '&recherche=' + recherche + '', '_blank');
    }
}
//fonction de chargement des factures non reglées (compta)
function load_facture_nonreglees(id_client_categ, id_niveau_relance) {
    var AppelAjax = new Ajax.Updater(
        "fac_liste_content",
...
//
function show_lettrage(class, contener) {
    var lettrage_selected = document.getElementsByClassName(class, contener);
    for (var i = 0; i < lettrage_selected.length; i++) {
        $(lettrage_selected[i]).style.backgroundColor = "#94B9D8";
    }
}

function hide_lettrage(class, contener) {
    var lettrage_selected = document.getElementsByClassName(class, contener);
    for (var i = 0; i < lettrage_selected.length; i++) {
...
function calcul_controle_caisse() {
    ...
    if ($("pass_tickresto").checked) {
        var toto_tr_saisie = parseFloat($("srtrsanstaux").innerHTML);
    } else {
        var toto_tr_saisie = parseFloat($("srtrsanstaux").innerHTML);
    }
    ...
    if ($("toto_tickresto_theorique_reel").innerHTML != "") {
        toto_theo += toto_tr_theo = parseFloat($("toto_tickresto_theorique_reel").innerHTML);
    }
    ...
    if ($("pass_tickresto").checked) {
        $("toto_tr_saisie").innerHTML = $("srtrsanstaux").innerHTML = $("montant_controle_tr").value = (parseFloat($("toto_tr_theorique").innerHTML).toFixed(tarifs_nb_decimales));
        $("toto_tr_saisie").style.color = "#999999";
    } else {
        $("toto_tr_saisie").innerHTML = $("srtrsanstaux").innerHTML = $("montant_controle_tr").value = toto_tr_saisie.toFixed(tarifs_nb_decimales);
        $("toto_tr_saisie").style.color = "#000000";
    }

    toto_saisie = (toto_esp_saisie + toto_chq_saisie + toto_cb_saisie + toto_tr_saisie).toFixed(tarifs_nb_decimales);
    ...
    $("diff_tr").innerHTML = $("diff_tickresto_net").innerHTML = $("montant_erreur_tr").value = (toto_tr_saisie - toto_tr_theo).toFixed(tarifs_nb_decimales);
    if (toto_tr_saisie - toto_tr_theo > 0) {
        $("diff_tr").innerHTML = $("diff_tickresto_net").innerHTML = "+" + (toto_tr_saisie - ttoto_tr_theo).toFixed(tarifs_nb_decimales);
    }

    toto_diff = (toto_esp_saisie - toto_esp_theo + toto_chq_saisie - toto_chq_theo + toto_cb_saisie - toto_cb_theo + toto_tr_saisie - toto_tr_theo).toFixed(tarifs_nb_decimales);
    ...
    if (toto_tr_theo.toFixed(tarifs_nb_decimales) != toto_tr_saisie.toFixed(tarifs_nb_decimales)) {
        $("diff_tr").style.color = "#FF0000";
        $("diff_tickresto").style.color = "#FF0000";
        $("diff_tickresto_net").style.color = "#FF0000";
        alerte_totaux = true;
    } else {
        $("diff_tr").style.color = "#000000";
        $("diff_tickresto").style.color = "#000000";
        $("diff_tickresto_net").style.color = "#000000";
    }
...
function calcul_transfert_caisse() {
    ...
    if (parseFloat(toto_cb_theo.toFixed(tarifs_nb_decimales)) < parseFloat(toto_cb_saisie.toFixed(tarifs_nb_decimales))) {
        $("TT_CB").innerHTML = toto_esp_theo.toFixed(tarifs_nb_decimales);
    }
    ...
    if (0 > parseFloat(toto_cb_saisie).toFixed(tarifs_nb_decimales)) {
        $("TT_CB").innerHTML = (0).toFixed(tarifs_nb_decimales);
    }
    toto_cb_saisie = parseFloat($("TT_CB").innerHTML);

    if ($("toto_chq_theorique").innerHTML != "") {
        toto_theo += toto_chq_theo = parseFloat($("toto_chq_theorique").innerHTML);
    }

    if ($("toto_tr_theorique").innerHTML != "") {
        toto_theo += toto_tr_theo = parseFloat($("toto_tr_theorique").innerHTML);
    }
    ...
    $("toto_cb_saisie").innerHTML = $("montant_transfert_cb").value = toto_cb_saisie.toFixed(tarifs_nb_decimales);
    $("toto_cb_saisie").style.color = "#000000";
    ...
    $("toto_tr_saisie").innerHTML = $("montant_transfert_tr").value = toto_tr_saisie.toFixed(tarifs_nb_decimales);
    $("toto_tr_saisie").style.color = "#000000";

    //toto_saisie = (toto_esp_saisie+toto_chq_saisie).toFixed(tarifs_nb_decimales);
    toto_saisie = (toto_esp_saisie + toto_chq_saisie + toto_tr_saisie + toto_cb_saisie).toFixed(tarifs_nb_decimales);
    ...
    $("diff_cb").innerHTML = $("RT_CB").innerHTML = (toto_cb_theo - toto_cb_saisie).toFixed(tarifs_nb_decimales);
    if (toto_cb_theo - toto_cb_saisie > 0) {
        $("diff_cb").innerHTML = "+" + (toto_cb_theo - toto_cb_saisie).toFixed(tarifs_nb_decimales);
    }
    ...
    $("diff_tr").innerHTML = (toto_tr_theo - toto_tr_saisie).toFixed(tarifs_nb_decimales);
    if (toto_tr_theo - toto_tr_saisie > 0) {
        $("diff_tr").innerHTML = "+" + (toto_tr_theo - toto_tr_saisie).toFixed(tarifs_nb_decimales);
    }

    //toto_diff = (toto_esp_theo-toto_esp_saisie + toto_chq_theo-toto_chq_saisie).toFixed(tarifs_nb_decimales);
    toto_diff = (toto_esp_theo - toto_esp_saisie + toto_chq_theo - toto_chq_saisie + toto_tr_theo - toto_tr_saisie).toFixed(tarifs_nb_decimales);
    ...
    var AppelAjax = new Ajax.Request(
        "compta_factures_client_" + mode + "_maj.php", {
            parameters: {
                ...
                id_edition_mode: id_edition_mode,
                id_niveau_relance: id_niveau_relance
            },
            evalScripts: true,
            onLoading: S_loading,
...
/****   Fonction Controle caisse ticket Restaurant  *********/
function calcul_val_tick(val_tic, nbr_tic, dev_reultat, total) {
    var sous_montnt = 0;
    var soldrl = 0;
    var soldrl_net = 0;
    var difftr = 0;
    var diff_net = 0;
    var nbr = 0;
    var taux = parseFloat($("taux_a_sup").value);
    var somme_a_sup = 0;
    sous_montnt = val_tic * nbr_tic;
    $(total).value = sous_montnt.toFixed(tarifs_nb_decimales);
    $(dev_reultat).innerHTML = sous_montnt.toFixed(tarifs_nb_decimales);
    nbr = parseFloat($("nombre").value);
    for (i = 1; i <= nbr; i++) {
        soldrl += parseFloat($("total_" + i).value);
    }
    $("srtr").innerHTML = soldrl.toFixed(tarifs_nb_decimales);
    $("solde_reel_tr").value = soldrl.toFixed(tarifs_nb_decimales);;
    somme_a_sup = (soldrl * taux) / 100;
    soldrl_net = soldrl - somme_a_sup;
    $("srtrsanstaux").innerHTML = soldrl_net.toFixed(tarifs_nb_decimales);
    $("solde_reel_tr_net").value = soldrl_net.toFixed(tarifs_nb_decimales);
    difftr = -(parseFloat($("montant_theorique_tr").value) - soldrl);
    $("diff_tickresto").innerHTML = difftr.toFixed(tarifs_nb_decimales);
    diff_net = soldrl_net - parseFloat($("montant_theorique_tr_reel").value);
    $("diff_tickresto_net").innerHTML = diff_net.toFixed(tarifs_nb_decimales);
    $("mt_dif_tr").value = difftr.toFixed(tarifs_nb_decimales);
    $("TT_TR").innerHTML = soldrl.toFixed(tarifs_nb_decimales);

}

function insert_new_line_tickresto() {
    var nbrline = parseInt($("nombre").value);
    var newnbr = parseInt(nbrline + 1);
    new Insertion.Bottom($("liste_tick_input"),
        '<div style="margin-left:40px;margin-bottom:5px">' +
        '<input type="hidden" value="0" id="total_' + newnbr + '" name="total_' + newnbr + '" />' +
        '<input type="text" name="val_' + newnbr + '" id="val_' + newnbr + '" class="classinput_nsize" size="5"/> * ' +
        '<input type="text" name="nbr_' + newnbr + '" id="nbr_' + newnbr + '" class="classinput_nsize" size="5"/> = ' +
        '<span id="aff_total_' + newnbr + '" style="float:right">  </span>' +
        '<SCRIPT type="text/javascript">' +
        'Event.observe($("nbr_' + newnbr + '"), "blur", function(evt){' +
        'calcul_val_tick ($("val_' + newnbr + '").value, $("nbr_' + newnbr + '").value,"aff_total_' + newnbr + '","total_' + newnbr + '") ;' +
        'Event.stop(evt); });' +
        '</SCRIPT> </div>'
    );
    $("nombre").value = newnbr;
...
function coche_line_gest_tr(type_action, second_id, length_list) {
...
//calcul des totaux TR
function calcul_telecollecte_tr() {

    var montant_tt_tr = 0;
    var count_ope = 0;
    indentation_exist_tr = parseInt($("indentation_exist_tr").value);

    for (j = 0; j <= indentation_exist_tr; j++) {
        if ($("CHK_EXIST_TR_" + j).checked) {
            montant_tt_tr += parseFloat($("CHK_EXIST_TR_" + j).value);
            count_ope++;
        }
    }

    $("TT_TR").innerHTML = $("aff_montant_total").innerHTML = $("toto_tr_saisie2").innerHTML = $("montant_total").value = montant_tt_tr.toFixed(tarifs_nb_decimales);

}
// equivalent de calcul_telecollecte_tp
function calcul_telecollecte_date_tr() {
...
//fonction d'affichage des resultats des paiements Ticket Restaurant
function etat_tr_result() {
    var AppelAjax = new Ajax.Updater(
        "etat_tr_result",
        "compta_tr_mouvement_result.php", {
            method: 'post',
            asynchronous: true,
            contentType: 'application/x-www-form-urlencoded',
            encoding: 'UTF-8',
            parameters: {
                recherche: '1',
                id_compte_tr: $("choix_id_caisse").value,
                page_to_show: $("page_to_show_s").value,
                date_debut: $("date_debut").value,
                date_fin: $("date_fin").value
            },
...
//fonction d'affichage de l'histyorique des télécollectes des Tickets Restaurant
function etat_telecollecte_result() {
    var AppelAjax = new Ajax.Updater(
        "etat_telecollecte_result",
        "compta_tr_telecollecte_historique_result.php", {
            method: 'post',
            asynchronous: true,
            contentType: 'application/x-www-form-urlencoded',
            encoding: 'UTF-8',
