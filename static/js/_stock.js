...
function imprimer_articles_fournisseur(Ref_fournisseur) {
    //alert("sdsdsd");	
    //alert("le ref est"+ Ref_fournisseur );

    window.open('imprime_articles_propose_fournisseur.php?ref_fournisseur=' + Ref_fournisseur + '&recherche=1' + '', '_blank');
...
function stock_etat_imprimer() {
    ...
    if ($("aff_info_tracab_s")) {
        if ($F("aff_info_tracab_s") == "1") {
            f_aff_info_tracab_s = "1";
        }
    }
...
//rï¿œsumï¿œ des numï¿œros de serie dans les mouvements de stock
    ...
    var AppelAjax = new Ajax.Updater(
        ...
        "stocks_a_transferer_result.php", {
            ...
            parameters: {
                recherche: '1',
                page_to_show: $F('page_to_show_s'),
                stock_depart: $F("stock_depart"),
                stock_arrivee: $F("stock_arrivee"),
                ref_art_categ: $F('ref_art_categ_s'),
                ref_constructeur: $F('ref_constructeur_s')
            },
            evalScripts: true,
